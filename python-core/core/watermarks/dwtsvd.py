...
class DwtSvdWatermark(BaseWatermark):
    ...
    def embed(self, carrier: Image.Image, payload: bytes, params: dict) -> tuple[Image.Image, dict]:
        ...
        strength = params.get('strength', 30.0)
        block_size = 8
        ...
        coeffs = pywt.wavedec2(lum, 'haar', level=1)
        ll = coeffs[0].copy()
        h, w = ll.shape
        ...
        if len(bits) > n_blocks:
            raise ValueError(f'DWT-SVD payload too large: need {len(bits)} blocks, only {n_blocks} available')
        ...
        for by in range(h_blocks):
            for bx in range(w_blocks):
                ...
                modified_ll[y1:y2, x1:x2] = (u * s) @ vt
                bi += 1

        coeffs_list = [modified_ll] + list(coeffs[1:])
        lum_reconstructed = pywt.waverec2(coeffs_list, 'haar')
        lum_reconstructed = np.clip(lum_reconstructed, 0, 255)
        dy = lum_reconstructed[:lum.shape[0], :lum.shape[1]] - lum
        for c in range(3):
            arr[:,:,c] += dy
